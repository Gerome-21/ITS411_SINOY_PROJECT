...
import { addItem } from './DashboardItemForm';
...
export default function ItemForm({ onClose }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await addItem(name, description, tagsArray);
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        ...
        <TouchableOpacity style={[styles.button, styles.submitButton]} onPress={handleSubmit} disabled={loading}>
          <Text style={styles.buttonText}>{loading ? 'Creating...' : 'Create Item'}</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
...
const styles = StyleSheet.create({
  ...
  hint: { color: '#666', fontSize: 12, marginBottom: 20, fontStyle: 'italic' },
